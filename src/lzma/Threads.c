...
{
  *p = CreateEvent(NULL, manualReset, (signaled ? True7z : FALSE), NULL);
  return HandleToWRes(*p);
}
...
WRes ManualResetEvent_Create(CManualResetEvent *p, int signaled) { return Event_Create(p, True7z, signaled); }
